...
if (!empty($search)) {
    $stmt = $conn->prepare("SELECT * FROM applications WHERE full_name LIKE ?");
    $searchParam = "%" . $search . "%";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['status'])) {
    if (!validateCsrfToken()) {
        echo "<div class='bg-red-500 text-white text-center p-2'>Token de seguridad invalido.</div>";
    } else {
        $id = (int)$_POST['id'];
        $status = $_POST['status'];

        $allowedStatuses = ['Aceptado', 'Rechazado', 'Pendiente'];
        if (in_array($status, $allowedStatuses)) {
            ...
            echo "<div class='bg-red-500 text-white text-center p-2'>Estado no valido</div>";
        }
    }
}
...
<head>
    ...
    <title>Gestion de Aplicaciones - Conductor Uber</title>
...
<body class="bg-black text-white font-sans">
    ...
    <div class="flex flex-col min-h-screen justify-center items-center px-4">
        <h1 class="text-4xl font-extrabold text-white mb-6 text-center">
            Gestion de Aplicaciones - <span class="text-blue-400">Conductor Uber</span>
        </h1>
                        ...
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr class="border-b border-gray-700">
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    <a
                                        href="?view_id=<?php echo (int)$row['id']; ?>"
        ...
        <?php if (isset($application_details)): ?>
            <div class="mt-6 w-full max-w-lg bg-gray-800 p-6 rounded-lg shadow-lg">
                <h2 class="text-2xl font-bold text-white mb-4">Detalles de la Aplicacion</h2>
                <p><strong>Nombre:</strong> <?php echo htmlspecialchars($application_details['full_name']); ?></p>
                <p><strong>Telefono:</strong> <?php echo htmlspecialchars($application_details['phone']); ?></p>
                ...
                <p><strong>Ano del Auto:</strong> <?php echo htmlspecialchars($application_details['car_year']); ?></p>
                <p><strong>Licencia:</strong> <?php echo htmlspecialchars($application_details['license']); ?></p>
                <p><strong>Por que seria buen conductor?:</strong> <?php echo htmlspecialchars($application_details['trustworthy']); ?></p>
