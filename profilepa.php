...
// Manejar la actualización del número de teléfono
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['phone'])) {
    ...
    if (!empty($newPhone)) {
        ...
        if ($stmt->execute()) {
            $phoneNotification = "Número de teléfono actualizado correctamente.";
            $userPhone = $newPhone; // Actualizar la variable en memoria
        } else {
            $phoneNotification = "Error al actualizar el número de teléfono.";
        }
    } else {
        $phoneNotification = "Por favor, ingresa un número de teléfono válido.";
    }
}

// Manejar el envío de nuevas direcciones
$notification = null;
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['type'], $_POST['address'])) {
    ...
    if (!empty($type) && !empty($address)) {
        $apiKey = "AIzaSyA3xKp9oCPeRfduSHv29G_nph7u4rLHQVI";
        ...
        if ($geoData['status'] === 'OK') {
            ...
            if ($stmt->execute()) {
                $notification = "¡Dirección agregada correctamente!";
            } else {
                $notification = "Error al agregar la dirección.";
            }
        } else {
            $notification = "No se pudieron calcular las coordenadas de la dirección.";
        }
    } else {
...
// Manejar la eliminación de direcciones
...
// Cargar direcciones predeterminadas desde la base de datos
    ...
    <div class="flex flex-col min-h-screen">
        ...
        <main class="flex-1 p-6">
            ...
            <section class="card p-6 rounded-xl shadow-md text-center">
                ...
                <p id="profile-phone" class="text-sm text-gray-400 mt-2">Teléfono: <?php echo htmlspecialchars($userPhone); ?></p>

                <!-- Formulario para editar el teléfono -->
                <form action="" method="POST" class="mt-4">
                    <input type="text" name="phone" placeholder="Nuevo número de teléfono" class="w-full px-4 py-2 border rounded-lg mb-2" required>
            ...
            <section class="mt-8">
                ...
                <form action="" method="POST" class="mt-6">
                    <input type="text" name="type" placeholder="Nombre de la dirección (Hogar, Trabajo...)" class="w-full px-4 py-2 border rounded-lg mb-2" required>
                    <input type="text" name="address" placeholder="Dirección" class="w-full px-4 py-2 border rounded-lg mb-2" required>
